...
# Creacion de usuario
def createRegistroUsuario(request):
    if request.method == 'POST':  
        print(request.POST)
        if not request.POST['first_name'] or not request.POST['last_name'] or not request.POST['email'] or not request.POST['password'] or not request.POST['username']:  
           return HttpResponse('Todos los campos son obligatorios')  
        else:
             createUsuario = User.objects.create_user(
...
def login(request):
    ...
    if user is not None:
        ...
        return render(request, 'clases.html', {'user': user})
    else:
      return HttpResponse('Por favor registrate.')
...
def moduloClase(request, NM):    
    ...
    categoriasClases = VideosCategoriaClase.objects.filter(categoriaClases__nombre_categoria=NM)
    nombre_categoria = categoriasClases[0].categoriaClases.nombre_categoria if categoriasClases.exists() else None
    return render(request, 'modulo_clase.html', {'categorias':categorias, 'categoriasClases':categoriasClases, 'nombre_categoria': nombre_categoria})
